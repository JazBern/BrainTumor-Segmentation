...
function nyul_hist_normalization( file_name_template, file_name_moving, data_from, data_to, file_mask_template , file_mask_moving)
...
% Save the reference after the scaling within a range (e.g. 1-256)
% save_untouch_nii(template,[ file_name_template(1:end-4) '_scaled.nii'] );
disp(strrep(file_name_moving, data_from, data_to));
save_untouch_nii(target, strrep(file_name_moving, data_from, data_to));