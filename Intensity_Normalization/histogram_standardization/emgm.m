function [label, model, llh] = emgm(X, init, prob_prior)
% Perform EM algorithm for fitting the Gaussian mixture model.
%   X: d x n data matrix
%   init: k (1 x 1) or label (1 x n, 1<=label(i)<=k) or center (d x k)
%  
%   prob_prior : if Flag 0, the labels are integer, Flag 1, the labels are
%   probabilities
%% initialization
fprintf('EM for Gaussian mixture: running ... \n');

R = initialization(X,init, prob_prior);
[~,label(1,:)] = max(R,[],2);
R = R(:,unique(label));
tol = 1e-30;
maxiter = 1500;
...
function R = initialization(X, init, prob_prior)
[d,n] = size(X);
